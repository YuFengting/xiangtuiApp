$(function () {
  'use strict';
  angular.module('xtui').controller('merrycontroller', merrycontroller);
  merrycontroller.$inject = ["$scope", "UtilService", "$rootScope", "UserService", "MerryService", "$ionicScrollDelegate", "$timeout", "$interval"];
  function merrycontroller($scope, UtilService, $rootScope, UserService, MerryService, $ionicScrollDelegate, $timeout, $interval) {
    ...
    $scope.getChristmasData = function () {
      MerryService.getChristmasIndexInfo("圣诞活动").then(function (res) {
    ...
    $scope.getChristmasList = function () {
      $scope.getChristmasData();
      MerryService.addChristmasActUser("圣诞活动").then(function (resadd) {
      });
      MerryService.getChristmasIndexList("圣诞活动").then(function (res) {
        if (res.status == '000000') {
    ...
    $scope.showmoney = function () {
      if ($scope.appleflag) {
        ...
        if ($scope.myapple == 0) {
          ...
          MerryService.getConsumeRewardLog("圣诞活动", $scope.appleToken).then(function (res) {
            if (res.status == "000000") {
              $scope.appleToken = res.token;
